...
const tools: Groq.Chat.Completions.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      name: 'search_web',
      description:
        "Search the live web for current information. Use this when the user's notes don't cover the topic, or when the question needs up-to-date information not likely to be in personal notes (news, current events, recent facts).",
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'The search query' },
...
async function executeToolCall(
  name: string,
  args: any
): Promise<{ result: string; resultSources: string[] }> {
...
export async function runAgent(userQuestion: string): Promise<{
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    for (const toolCall of responseMessage.tool_calls) {
      ...
      const { result, resultSources } = await executeToolCall(
